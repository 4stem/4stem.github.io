---
layout: post
title: "Μοριοδότηση 2018"
description: "Φυσική Γ' Λυκείου"
category: Άσκηση
tags: [Βαθμολογικό]
---
{% include JB/setup %}

**`Ενδεικτικές απαντήσεις και από γραπτά μαθητών`**




**`Θέμα Α`**

**Α1** - $$γ$$

**Α2** - $$δ$$

**Α3** - $$α$$

**Α4** - $$δ$$

**Α5:**  $$Λ - Σ - Λ - Σ - Λ$$




**`Θέμα Β`**


**B1**-$$(i)$$

![σχήμα]({{ site.url }}/assets/b1_kyma.png) 

$$d_2 = \sqrt{d_1^2 + d^2} = \sqrt{4 \cdot λ_1^2 + \frac{9}{4} \cdot λ_1^2 } = \frac{5 \cdot λ_1}{2}$$

Ίδιο υλικό 

$$υ_δ = λ_1 \cdot f_1 = λ_2 \cdot f_2 \overset{f_2=2\cdot f_1}{\implies} λ_2 = \frac{λ_1}{2} $$


α) $$ \underline {τρόπος} $$

$$|A_Σ| = |2 A \cdot συν \frac{2π (d_1 - d_2)}{2 λ_2} | = |2 A \cdot συν \frac{π (\frac{3λ_1}{2} - \frac{5λ_1}{2} )}{\frac{λ_1}{2} }| = |2Α|$$

β) $$ \underline {τρόπος} $$


$$\left. \begin{matrix}d_1 - d_2 = \frac{3λ_1}{2} - \frac{5λ_1}{2} = -λ_1 = -2λ_2 \\\\ d_1 - d_2 = N \cdot λ_2\end{matrix} \right\} Ν = -2 \quad ενίσχυση$$

**Β2** - $$(ιιι)$$

![σχήμα]({{ site.url }}/assets/b2_kyklos.png) 

m: $$Σ\vec{τ_{εξ(Κ)}} = \vec{0} \Rightarrow Δ\vec{L} = \vec{0} \Rightarrow \vec{L_1} = \vec{L_2}$$

Η τάση του νήματος διέρχεται από τον άξονα περιστροφής

α) $$ \underline {τρόπος} $$

Άρα $$m \cdot υ_1 \cdot R = m \cdot υ_2 \cdot \frac{R}{2} \Rightarrow υ_2 = 2 υ_1$$

$$ΘΜΚΕ_m (Σ \rightarrow Σ') \quad Κ_{Σ'} - Κ_{Σ} = W_F \Rightarrow \frac{1}{2} \cdot m \cdot υ_2^2 - \frac{1}{2} \cdot m \cdot υ_1^2 = W_F$$

$$\left. \begin{matrix} W_F = \frac{3}{2} \cdot m \cdot υ_1^2 \\\\ υ_1 = ω \cdot R\end{matrix} \right\} W_F = \frac{3}{2} \cdot m \cdot ω^2 \cdot R^2$$

β) $$ \underline {τρόπος} $$

$$I_1 \cdot ω = Ι_2 \cdot ω´ \Rightarrow m \cdot R^2 \cdot ω = m \cdot \frac{R^2}{4} \cdot ω´ \Rightarrow ω´ = 4ω$$

$$ΘΜΚΕ_m (Σ \rightarrow Σ') \quad Κ_{Σ'} - Κ_{Σ} = W_F \Rightarrow \frac{1}{2} \cdot Ι_2 \cdot ω´^2 - \frac{1}{2} \cdot Ι_1 \cdot ω^2 = W_F \Rightarrow W_F = \frac{1}{2}m \frac{R^2}{4}16ω^2 - \frac{1}{2}m \cdot R^2 ω^2 \Rightarrow W_F = \frac{3}{2} \cdot m \cdot ω^2 \cdot R^2 $$


**Β3** - $$(ι)$$

Εξίσωση Bernoulli για μια ρευματική γραμμή $$(Γ \rightarrow Δ)$$

$$P_Γ + \frac{1}{2}ρ \cdot υ_Γ^2 = P_Δ + \frac{1}{2}ρ \cdot υ_Δ^2 + ρ \cdot g \cdot h$$

Εξίσωση συνέχειας $$(Γ \rightarrow Δ)$$

$$Π_Γ = Π_Δ \Rightarrow Α_Γ \cdot υ_Γ = Α_Δ \cdot υ_Δ \overset{Α_Γ = 2Α_Δ}{\implies} υ_Δ = 2υ_Γ$$

Οριζόντια βολή $$(Δ \rightarrow Κ)$$

$$\left. \begin{matrix} h = \frac{1}{2} g \cdot t^2 \\\\ 4h = υ_Δ \cdot t\end{matrix} \right\} 4h = υ_Δ \cdot \sqrt{\frac{2h}{g}} \Rightarrow υ_Δ^2 = 8g\cdot h \overset{υ_Δ = 2υ_Γ}{\implies} 4υ_Γ^2 = 8g\cdot h \Rightarrow υ_Γ^2 = 2g \cdot h \Rightarrow g\cdot h = \frac{υ_Γ^2}{2}$$


Άρα η εξίσωση Bernoulli γράφεται

$$P_Γ - P_Δ =  \frac{1}{2}ρ \cdot υ_Δ^2 + ρ \cdot g \cdot h - \frac{1}{2}ρ \cdot υ_Γ^2 =   \frac{1}{2}ρ \cdot 4 υ_Γ^2 + ρ \cdot \frac{υ_Γ^2}{2} - \frac{1}{2}ρ \cdot υ_Γ^2 = 2 ρ \cdot υ_Γ^2$$


**`Θέμα Γ`**


**Γ1**

![σχήμα]({{ site.url }}/assets/dyo_elatiria.png) 

$$k_1 = k_2 = k$$

$$m_1 = m_2 = m$$

$$Δl = 0.4 m = A_1$$

$$K_1-m, \quad ΑΑΤ: D_1 = k_1 = m_1 \cdot ω_1^2 \Rightarrow ω_1 = \sqrt{\frac{k}{m}} = 5 \frac{rad}{sec}$$

$$υ_{max1} = ω_1 \cdot A_1 = \sqrt{\frac{k}{m}} \cdot Δl = 2 \frac{m}{sec}$$

$$f_1 = \frac{υ_{ηχ} - υ_{max1}}{υ_{ηχ}} \cdot f_s$$

$$ΑΔΟ \quad m_1, m_2 \quad (Θ.Ι.) \quad m_1 \cdot υ_{max1} = (m_1 + m_2) \cdot V \Rightarrow V = 1 \frac{m}{sec}$$

$$f_2 = \frac{υ_{ηχ} - V}{υ_{ηχ}} \cdot f_s$$

$$\frac{f_1}{f_2} = \frac{υ_{ηχ} - υ_{max1}}{υ_{ηχ}-V} = \frac{338}{339}$$

**Γ2**

$$(m_1+m_2):$$

Στη θέση Θ.Φ.Μ. $$ΣF=0$$ άρα αυτή είναι και Θ.Ι.

$$Τ.Θ.: ΣF = -F_{ΕΛ1} - F_{ΕΛ2} = -k_1 \cdot x - k_2 \cdot x = - (2k)x$$

Για να εκτελεί ένα σώμα ΑΑΤ πρέπει να ισχύει

$$ΣF = -D \cdot x, \quad Άρα \quad D = 2k = (m_1 + m_2) ω^2 \Rightarrow ω = \sqrt{\frac{2k}{2m}} = \sqrt{\frac{k}{m}} = 5 \frac{rad}{sec}, \quad T = \frac{2π}{ω}= \frac{2π}{5} sec$$

$$Θ.Ι.: V = υ_{max} \overset{υ_{max} = ω \cdot A}{\implies} 1 = 5 \cdot A \Rightarrow A = 0.2m$$

**Γ3**

$$\left. \begin{matrix} h = f_{ΔΕΚΤΗ} = f_s \\\\  f_{ΔΕΚΤΗ} = \frac{υ_{ηχ} \pm υ_{ΣΥΣ}}{υ_{ηχ}} \cdot f_s\end{matrix} \right\} υ_{ΣΥΣ} = 0$$

Για πρώτη φορά, δηλαδή ακραία θέση, οπότε

$$Δt = \frac{T}{4} = \frac{π}{10} sec$$

**Γ4**

$$|\frac{dp}{dt}|_{m_1+m_2(max)} = ΣF_{max} = D \cdot A \overset{D = 100}{\implies} ΣF_{max}=20N, \quad ή \quad \frac{kg \cdot m}{sec^2}$$



**`Θέμα Δ`**

![σχήμα]({{ site.url }}/assets/yper.png) 

Ράβδος (ρ)

$$Μ = 8kg$$

$$l = 3m$$

Δίσκος (Δ)

$$m_Δ = 4kg$$

$$R_Δ = \frac{\sqrt{2}}{2} m$$

Τροχαλία (τροχ)

$$R = 0.2 m$$

$$I_{τροχ} = 1.95 kg \cdot m^2$$

Κύλινδρος

$$m = 30 kg$$

$$R = 0.2 m$$

$$ημφ = 0.8 $$

$$συνφ = 0.6 $$

$$g = 10 \frac{m}{sec^2}$$


**Δ1**

$$I_{ρ-Δ} = (\frac{1}{12} \cdot M \cdot l^2 + M \frac{l^2}{4}) + \frac{1}{12} \cdot m_Δ \cdot R_Δ^2 = 25 kg \cdot m^2$$


**Δ2**

![σχήμα]({{ site.url }}/assets/d2_rab.png) 


$$|\frac{dL}{dt}|_{ρ-Δ} = Στ_{(0)} = W_ρ \cdot \frac{l}{2} \cdot συνφ = 72 \frac{kg \cdot m^2}{sec^2} \quad ή \quad N \cdot m$$

**Δ3**

α) $$ \underline {τρόπος} $$

$$ΑΔΜΕ_{ρ-Δ} (Ι \rightarrow II): K_I + U_1 = K_{II} + U_{II} \Rightarrow 0 + (-M \cdot g \cdot \frac{l}{2} \cdot ημφ + U_{βαρ(Δ)(Ι))}) = Κ_{ΙΙ} + (-Μ \cdot g \cdot \frac{l}{2}  + U_{βαρ(Δ)(ΙI))})$$

$$K_{II} = M \cdot g \cdot \frac{l}{2} \cdot (1 - ημφ) \Rightarrow K_{II} = 24J$$



**Δ4**

![σχήμα]({{ site.url }}/assets/d4.png) 

νήμα(2) αβαρές, μη εκτατό $$(Τ_2 = Τ'_2)$$

ΚΧΟ:

$$υ_Β = 2 \cdot υ_{cm} = 2 \cdot ω \cdot R \Rightarrow α_Β = 2 \cdot α_{cm} = 2 \cdot α_{γων} \cdot R$$

$$υ_Β = υ_{H} =  ω_{τροχ} \cdot R \Rightarrow α_Β = α_{Η} = α_{γων_{τροχ}} \cdot R$$

$$m: \quad MET \quad ΣF_x = m \cdot α_x \Rightarrow W_x - T_{στ} - Τ_2 = m \cdot α_{cm} \quad (1)$$

$$ΣΤΡΟΦ. \quad Στ = Ι \cdot α_{γων} \Rightarrow T_{στ} \cdot R - T_2 \cdot R = \frac{1}{2} \cdot m \cdot R^2 \cdot α_{γων} \overset{α_{cm} = α_{γων} \cdot R}{\implies} T_{στ} - Τ_2 = \frac{1}{2} \cdot m \cdot α_{cm} \quad (2)$$

$$(1)Λ(2) \Rightarrow W_x - 2T_2 = \frac{3}{2} \cdot m \cdot α_{cm} \quad (3)$$

$$τροχ: \quad Στ = Ι \cdot α_{γων} \Rightarrow T'_2 \cdot R = 1.95 \cdot α_{γων(τροχ)} \overset{α_{γων(τροχ)} = \frac{2 α_{cm}}{R}}{\underset{(3)}{\implies}} \\\ $$


$$W_x - 2 \cdot \frac{1.95 \cdot \frac{2 α_{cm}}{R}}{R}  = \frac{3}{2} \cdot m \cdot α_{cm}$$

$$300 \cdot 0.8 - \frac{4 \cdot 1.95 \cdot α_{cm}}{4 \cdot 10^{-2}} = 45 \cdot α_{cm} \Rightarrow α_{cm} = 1 \frac{m}{sec^2}$$

$$α_{cm} = α_{γων} \cdot R \Rightarrow α_{γων} = 5 \frac{rad}{sec^2}$$

κύλινδρος:

α) $$ \underline {τρόπος} $$

$$s = \frac{1}{2} \cdot α_{cm} \cdot t^2 \Rightarrow t = 2 sec$$

$$υ_{cm} = α_{cm} \cdot t \Rightarrow υ_{cm} = 2 \frac{m}{sec}$$

β) $$ \underline {τρόπος} $$

$$ΘΜΚΕ_{Ο \rightarrow S}: K_{τελ} - Κ_{αρχ} = ΣW \Rightarrow (\frac{1}{2} \cdot m \cdot υ_{cm}^2 + \frac{1}{2} \cdot m \cdot R^2 \cdot ω^2) -0 = (ΣF_x) \cdot S + (Στ) \cdot Δθ$$

$$ΣF_x = m \cdot α_{cm}$$

$$Στ = Ι \cdot α_{γων}$$

$$\frac{3}{4} \cdot m \cdot υ_{cm}^2 = m \cdot α_{cm} \cdot s + \frac{1}{2} \cdot m \cdot R^2 \cdot α_{γων} \cdot Δθ \overset{α_{γων} \cdot R = α_{cm}}{\underset{Δθ \cdot R = S}{\implies}} \\\ $$

$$\frac{3}{4} \cdot m \cdot υ_{cm}^2 = m \cdot α_{cm} \cdot s + \frac{1}{2} \cdot m \cdot α_{cm} \cdot s \Rightarrow \frac{3}{4} \cdot m \cdot υ_{cm}^2 = \frac{3}{2} \cdot α_{cm} \cdot S \Rightarrow υ_{cm} = 2 \frac{m}{sec}$$

Μπορείτε να εκτυπώσετε τις λύσεις σε μορφή pdf από [εδώ]({{ site.url }}/assets/m2018.pdf) 